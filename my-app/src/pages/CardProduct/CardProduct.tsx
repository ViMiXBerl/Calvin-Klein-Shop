...
import { fetchCartProduct } from "modules/redux/actions";
...
const CardProduct = ({ match }: RouteComponentProps<CardProductProps>): any => {
	...
	const { product, loading, error } = productCart;

	useEffect((): any => {
		dispatch(fetchCartProduct(match.params.id));
	}, [dispatch, match]);
